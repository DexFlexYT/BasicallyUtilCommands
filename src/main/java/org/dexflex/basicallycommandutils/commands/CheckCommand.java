...
public class CheckCommand {
                                ...
                                .then(CommandManager.argument(
                                                ...
                                                .executes(ctx -> {
                                                    Predicate<CachedBlockPosition> filter =
                                                            BlockPredicateArgumentType.getBlockPredicate(ctx, "block_filter");
    ...
    private static int executeBlocks(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendFeedback(
                () -> Text.literal(
                        "Found " + found + " matching blocks within " + dist + " blocks."
                ),
                false
        );
        return found;
    ...
    private static int executeIntersection(CommandContext<ServerCommandSource> ctx)
        ...
        Collection<? extends Entity> ents =
                EntityArgumentType.getEntities(ctx, "selector");

        int hits = 0;
        for (Entity e : ents) {
            if (e.getBoundingBox().contains(x, y, z)) {
                hits++;
            }
        }
        ...
        source.sendFeedback(
                () -> Text.literal("Found " + result + " intersections."),
                false
        );
    ...
    private static int executeCollision(CommandContext<ServerCommandSource> ctx,
        ...
        Vec3d offset = source.getPosition().subtract(target.getPos());
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (filter != null &&
                            !filter.test(new CachedBlockPosition(world, pos, true))) {
                        continue;
                    }
