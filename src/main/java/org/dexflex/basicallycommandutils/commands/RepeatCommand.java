...
public class RepeatCommand {
                        ...
                        .then(CommandManager.argument("delay", IntegerArgumentType.integer(0))
                                .then(CommandManager.literal("run")
                                        .fork(dispatcher.getRoot(), ctx -> {
                                            ...
                                            java.util.Collection<ServerCommandSource> sources = new ArrayList<>();
                                            ...
                                            if (delay == 0) {
                                                ...
                                                String fullCommand = ctx.getInput();
                                                int runIndex = fullCommand.indexOf("run") + 4;
                                                String commandPart = fullCommand.substring(runIndex);
    ...
    private static void registerTickHandler() {
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            if (scheduledExecutions.isEmpty()) return;
